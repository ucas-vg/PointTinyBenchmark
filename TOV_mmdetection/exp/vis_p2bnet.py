...
gt_path = "/home/pfchen/disk1/cpf/P2BNet/biye/coco_1200_latest_pseudo_ann_1_pbr.json"
...
for img_name in show_img[:300]:
    ...
    if gt_or_result == 1:
        ...
        for i, bb in enumerate(bbox):
            bb = [int(x) for x in bb]
            top_left, bottom_right = bb[:2], [bb[0] + bb[2], bb[1] + bb[3]]
            img = cv2.rectangle(img, tuple(top_left), tuple(bottom_right), (0,229,238), 4)  # (0,229,238)(127, 255, 0)
            cla = classes[i]
            sco = scores[i]
    ...
    # 保存图片
    plt.imshow(img)
    if not os.path.exists('vis_100_pt_bbox_p2b++_' + name_out):
        os.makedirs('vis_100_pt_bbox_p2b++_' + name_out)
    if gt_or_result == 1:
        plt.savefig(os.path.join('vis_100_pt_bbox_p2b_' + name_out, img_name.split('.')[0]+'.jpg'), bbox_inches='tight')
    else:
        plt.savefig(os.path.join('vis_100_pt_bbox_p2b_' + name_out, img_name.split('.')[0]+'.jpg'), bbox_inches='tight')