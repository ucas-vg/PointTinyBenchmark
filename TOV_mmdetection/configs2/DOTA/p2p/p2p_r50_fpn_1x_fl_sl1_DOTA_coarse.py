...
test_pipeline = [
    ...
    dict(
        ...
        tile_shape=(1024, 1024),  # sub image size by cropped
        tile_overlap=(200, 200),
        scale_factor=[1.0],
        flip=False,
        transforms=[
...
data = dict(
    samples_per_gpu=4,  # 2
    workers_per_gpu=8,  # didi-debug 2
    ...
    train=dict(
        ...
        img_prefix=data_root + 'DOTA-split/trainsplit/images',  # 'train2017/',
    ...
    val=dict(
        ...
        ann_file=data_root + "DOTA/val/DOTA_val.json",
        img_prefix=data_root + 'DOTA/val/images/images',  # 'images/',  # 'val2017/',
        pipeline=test_pipeline),
    test=dict(
        type=dataset_type,
        ann_file=data_root + 'DOTA/val/DOTA_val.json',
        img_prefix=data_root + 'DOTA/val/images/images',
        pipeline=test_pipeline))
...
model = dict(
    ...
    bbox_head=dict(
        ...
        in_channels=256,
        feat_channels=256,
        stacked_convs=3,  # didi4 TODO: 论文中“three stacked convolutions”
        strides=[16],  # [4, 8, 16, 32, 64] # [8, 16, 32, 64, 128]
        # point_anchor=[(-0.25, -0.25), (0.25, -0.25), (0.25, 0.25), (-0.25, 0.25)],  # Grid
        point_anchor=[(0.0, 0.0)],
        loss_cls=dict(
            type='FocalLoss',
            use_sigmoid=True,
            gamma=2.0,
...
work_dir = '/home/pfchen/yxh/TOV_mmdetection_cache/work_dir/DOTA/P2P/p2p_r50_fpn_1x_fl_sl1_DOTA_coarse_top5_lr1e-4_s16_coarse'

# work_dir = 'work_dirs/dbg_dota'