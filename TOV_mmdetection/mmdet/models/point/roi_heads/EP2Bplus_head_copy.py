...
class EP2BplusHead(StandardRoIHead):
    ...
    def _bbox_forward_train(self, x, proposal_list_base, proposals_list, proposals_valid_list, neg_proposal_list,
        ...
        if reg_box is not None:
            ...
            # iou_ori = bbox_overlaps(bboxes_pred, pseudo_boxes.unsqueeze(1).expand(reg_box.shape),
            #                         is_aligned=True).reshape(*reg_box.shape[:2])
            #
            # neg_weights = self.ass(neg_proposal_list, bboxes_pred, gt_points)
            #
            # if self.cluster_mode == 'upper':
            #
            #     iou_max, idx = iou.max(dim=1)
            #     pseudo_boxes_reg = bboxes_pred[torch.arange(len(bboxes_pred)), idx]
            #     bbox_results['pseudo_boxes'] = pseudo_boxes_reg.split([len(i) for i in gt_points])
            #     mean_ious_reg = [iou_max.mean()]
            # elif self.cluster_mode == 'classification':
            #     bboxes_pred_ = bboxes_pred.split([len(i) for i in gt_points])
            #     bboxes_pred_ = [i.reshape(-1, 4) for i in bboxes_pred_]
            #     rois_reg = bbox2roi(bboxes_pred_)
            #     rois_all = torch.cat([rois_reg, neg_rois], dim=0)
            #     bbox_feats = self.bbox_roi_extractor(
            #         x[:self.bbox_roi_extractor.num_inputs], rois_all)
            #     cls_scores = self.bbox_head.forward_retrain_cls(bbox_feats)
            #
            #     ## cal classification loss
            #     loss_retrain_cls = self.bbox_head.loss_retrain_cls(cls_scores, gt_labels, dynamic_weight.sum(dim=-1),
            #                                                        neg_weights, num_pos=len(rois_reg))
            #
            #     ## choose the max score bbopx
            #     pos_scores = cls_scores[:len(rois_reg)]
            #     pos_scores = pos_scores.reshape(len(gt_labels), -1, pos_scores.shape[-1]).sigmoid()
            #     pos_scores = pos_scores[range(len(gt_labels)), :, gt_labels]
            #
            #     ###
            #
            #     _, idx = pos_scores.max(dim=1)
            #     pseudo_boxes_reg = bboxes_pred[range(len(bboxes_pred)), idx]
            #
            #     ## choose IoU
            #     iou_choose = iou[range(len(bboxes_pred)), idx]
            #
            #     mean_ious_reg = [iou_choose.mean()]
            #     ## cal IoU with gt
            # elif self.cluster_mode == 'mil_cls':
            #     pass  # TODO
            #     bboxes_pred_ = bboxes_pred.split([len(i) for i in gt_points])
            #     bboxes_pred_ = [i.reshape(-1, 4) for i in bboxes_pred_]
            #     rois_reg = bbox2roi(bboxes_pred_)
            ...
            #         x[:self.bbox_roi_extractor.num_inputs], rois_all)
            #
            #     fc_feats = self.bbox_head.forward_retrain_2fcs(bbox_feats)
            #     cls_scores = self.bbox_head.forward_retrain_cls(fc_feats)
            #     ins_scores = self.bbox_head.forward_retrain_ins(fc_feats[:-len(neg_rois)])
            #
            #     ## cal classification loss
            #     loss_retrain_cls = self.bbox_head.loss_retrain_cls(cls_scores, gt_labels, dynamic_weight.sum(dim=-1),
            #                                                        neg_weights, num_pos=len(rois_reg),
            ...
            #     pos_scores = cls_scores[:len(rois_reg)]
            #     pos_scores = pos_scores.reshape(len(gt_labels), -1, pos_scores.shape[-1]).sigmoid()
            #     pos_scores_ = pos_scores[range(len(gt_labels)), :, gt_labels]
            #
            #     ###
            #     iou_merge = bbox_overlaps(bboxes_pred, pseudo_boxes.unsqueeze(1).expand(reg_box.shape), is_aligned=True)
            #
            #     # _, idx = pos_scores_.max(dim=1)
            #     _, idx = (pos_scores_ * (iou_merge > 0.8)).max(dim=1)
            #     pseudo_boxes_reg = bboxes_pred[range(len(bboxes_pred)), idx]
            #
            #     ## choose IoU
            #     iou_choose = iou[range(len(bboxes_pred)), idx]
            ...
            #     iou_max, idx = iou.max(dim=1)
            #     mean_ious_reg_max = [iou_max.mean()]
            # elif self.cluster_mode == 'mil_cls_sampling_mode1':
            #     bboxes_pred_ = bboxes_pred.split([len(i) for i in gt_points])
            #     bboxes_pred_ = [i.reshape(-1, 4) for i in bboxes_pred_]
            #
            #     ## mode1 sampling
            #     sample_num = 10
            #     maxv, _ = iou_ori.max(dim=1)
            #     minv, _ = iou_ori.min(dim=1)
            #     stp = (maxv - minv) / sample_num
            #     sv = minv[:, None] + stp[:, None] * torch.arange(sample_num).to(iou.device)
            #     si = (sv[:, :, None] - iou_ori[:, None, :]).abs().argmin(dim=-1)
            #
            #     si = torch.stack([iou.max(1)[1], iou.max(1)[1]], dim=1)
            #     sk = torch.stack([iou_ori.max(1)[1], iou_ori.max(1)[1]], dim=1)
            #     ## feat forward
            #     rois_reg = bbox2roi(bboxes_pred_)
            #     rois_all = torch.cat([rois_reg, neg_rois], dim=0)
            #     bbox_feats = self.bbox_roi_extractor(
            ...
            #     ## cal classification loss
            #     pos_scores = cls_scores[:len(rois_reg)]
            #     neg_scores = cls_scores[len(rois_reg):]
            #     cls = pos_scores.reshape(len(gt_labels), -1, pos_scores.shape[-1])[
            #         torch.arange(iou.shape[0])[:, None], si]
            #     ins = ins_scores.reshape(len(gt_labels), -1, ins_scores.shape[-1])[
            #         torch.arange(iou.shape[0])[:, None], si]
            #     cls_scores = torch.cat([cls.reshape(-1, cls.shape[-1]), neg_scores])
            #     # dynamic_weight.sum(dim=-1)
            #     # dynamic_weight.new_full((dynamic_weight.shape[0],), 1)
            #     loss_retrain_cls,acc = self.bbox_head.loss_retrain_cls(cls_scores, gt_labels,dynamic_weight.sum(dim=-1),
            #                                                        neg_weights, num_pos=si.shape[0] * si.shape[1],
            #                                                        ins_scores=ins)
            #     # cls = pos_scores.reshape(len(gt_labels), -1, pos_scores.shape[-1])[
            #     #     torch.arange(iou.shape[0])[:, None], sk]
            #     # ins = ins_scores.reshape(len(gt_labels), -1, ins_scores.shape[-1])[
            #     #     torch.arange(iou.shape[0])[:, None], sk]
            #     # gt_labels_=torch.clone(gt_labels)
            #     # gt_labels_[:] = 80
            #     # cls_scores = torch.cat([cls.reshape(-1, cls.shape[-1]), neg_scores])
            #     # loss_retrain_cls1 = self.bbox_head.loss_retrain_cls(cls_scores, gt_labels_, dynamic_weight.sum(dim=-1),
            #     #                                                     neg_weights, num_pos=si.shape[0] * si.shape[1],
            #     #                                                     ins_scores=ins)
            #     # loss_retrain_cls = loss_retrain_cls + loss_retrain_cls1
            #     ## choose the max score bbopx
            #
            #     pos_scores = pos_scores.reshape(len(gt_labels), -1, pos_scores.shape[-1]).sigmoid()
            #     pos_scores_ = pos_scores[range(len(gt_labels)), :, gt_labels]
            ...
            #     pseudo_boxes_reg = bboxes_pred[range(len(bboxes_pred)), idx]
            #
            #     # _, idx = cls.sigmoid()[range(len(gt_labels)), :, gt_labels].max(dim=1)
            #     # iou_choose=iou[torch.arange(len(bboxes_pred))[:, None], si][range(len(bboxes_pred)), idx]
            #     # pseudo_boxes_reg =bboxes_pred[torch.arange(len(bboxes_pred))[:, None], si][range(len(bboxes_pred)), idx]
            #
            #     ## choose IoU
            #     iou_choose = iou[range(len(bboxes_pred)), idx]
            #
            ...
            #     ## cal IoU with gt
            #
            #     ins_scores_ = ins_scores.reshape(len(gt_labels), -1, pos_scores.shape[-1]).softmax(dim=-2)
            #     ins_scores_ = ins_scores_[range(len(gt_labels)), :, gt_labels]
            #     cls = (ins_scores_ * pos_scores_)
            #     iou_choose=iou[range(len(bboxes_pred)), cls.max(1)[1]]
            #
            #     mean_ious_reg = [iou_choose.mean()]
            #     iou_max, idx = iou.max(dim=1)
            #     mean_ious_reg_max = [iou_max.mean()]
            # elif self.cluster_mode == 'mil_cls_sampling_mode2':
            #     bboxes_pred_ = bboxes_pred.split([len(i) for i in gt_points])
            #     bboxes_pred_ = [i.reshape(-1, 4) for i in bboxes_pred_]
            #
            #     ## mode1 sampling
            #     sample_num = 10
            #     si = iou_ori.argsort(dim=1)[torch.arange(len(iou_ori))[:, None], torch.linspace(0, iou_ori.shape[1] - 1,
            #                                                                                     steps=sample_num).long()]
            #
            #     ## feat forward
            #     rois_reg = bbox2roi(bboxes_pred_)
            #     rois_all = torch.cat([rois_reg, neg_rois], dim=0)
            ...
            #     iou_max, idx = iou.max(dim=1)
            #     mean_ious_reg_max = [iou_max.mean()]
            # elif self.cluster_mode == 'mil_cls_sampling_mode3':
            #     bboxes_pred_ = bboxes_pred.split([len(i) for i in gt_points])
            #     bboxes_pred_ = [i.reshape(-1, 4) for i in bboxes_pred_]
            #
            #     ## mode1 sampling
            ...
            #     cls_scores = self.bbox_head.forward_retrain_cls(fc_feats)
            #     ins_scores = self.bbox_head.forward_retrain_ins(fc_feats[:-len(neg_rois)])
            #     neg_ins_scores = self.bbox_head.forward_retrain_neg_ins(fc_feats[:-len(neg_rois)])
            #
            #     ## cal classification loss
            #     pos_scores = cls_scores[:len(rois_reg)]
            #     neg_scores = cls_scores[len(rois_reg):]
            ...
            #     ins = ins_scores.reshape(len(gt_labels), -1, ins_scores.shape[-1])[
            #         torch.arange(iou.shape[0])[:, None], si]
            #     ins_neg = neg_ins_scores.reshape(len(gt_labels), -1, neg_ins_scores.shape[-1])[
            #         torch.arange(iou.shape[0])[:, None], si]
            #     cls_scores = torch.cat([cls.reshape(-1, cls.shape[-1]), neg_scores])
            #     # dynamic_weight.sum(dim=-1)
            #     # dynamic_weight.new_full((dynamic_weight.shape[0],), 1)
            #     loss_retrain_cls, acc = self.bbox_head.loss_retrain_cls_2(cls_scores, gt_labels,
            #                                                             dynamic_weight.sum(dim=-1),
            #                                                             neg_weights, num_pos=si.shape[0] * si.shape[1],
            #                                                             ins_scores=ins,neg_ins_scores=ins_neg)
            #
            #
            #     pos_scores = pos_scores.reshape(len(gt_labels), -1, pos_scores.shape[-1]).sigmoid()
            #     pos_scores_ = pos_scores[range(len(gt_labels)), :, gt_labels]
            ...
            #     iou_max, idx = iou.max(dim=1)
            #     mean_ious_reg_max = [iou_max.mean()]
            # elif self.cluster_mode == 'cluster':
            #     bboxes_pred_ = [i.reshape(-1, 4) for i in bboxes_pred_]
            #     rois_reg = bbox2roi(bboxes_pred_)
            #     rois_all = torch.cat([rois_reg, neg_rois], dim=0)
            #     bbox_feats = self.bbox_roi_extractor(
            ...
            #     pos_scores = pos_scores[range(len(gt_labels)), :, gt_labels]
            #
            #     graph = bbox_overlaps(bboxes_pred, bboxes_pred)
            #     # graph *= (pos_scores > 0.3).unsqueeze(1).expand(graph.shape)
            #     graph_ = (graph > 0.7).sum(-1) / graph.shape[-1]
            #     cost = graph_ + pos_scores
            #     _, idx = cost.max(dim=1)
            #     pseudo_boxes_reg = bboxes_pred[range(len(bboxes_pred)), idx]
            #
            #     ## choose IoU
            #     iou_choose = iou[range(len(bboxes_pred)), idx]
            ...
            #
            #     graph = bbox_overlaps(bboxes_pred, bboxes_pred)
            #     _, cluster1 = (graph > 0.9).sum(-1).max(1)
            #     remove = graph[range(len(bboxes_pred)), cluster1] < 0.9
            #     remove = remove.unsqueeze(1).expand(graph.shape)
            #     l, k = ((graph * remove) > 0.9).sum(-1).max(1)
            #     fff = torch.stack([iou[range(len(bboxes_pred)), cluster1], iou[range(len(bboxes_pred)), k]]).max(0)[
            #         0].mean()
            #     mean_ious_reg = [fff]
