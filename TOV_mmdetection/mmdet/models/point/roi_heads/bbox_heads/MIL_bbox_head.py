...
class Shared2FCInstanceMILHead(Shared2FCBBoxHead):
    ...
    def __init__(self,
        ...
        self.with_neg_ins=with_neg_ins
        ...
        for i in range(self.num_stages):
            ...
            if self.with_ins:
                self.fc_ins.append(build_linear_layer(
                    ...
                    in_features=self.cls_last_dim,
                    out_features=num_cls))
            if self.with_neg_ins:
                self.fc_neg_ins.append(build_linear_layer(
                    self.cls_predictor_cfg,
                    in_features=self.cls_last_dim,
                    out_features=num_cls))
            if self.with_reg:
        ...
        if stage_mode == 'CBP':
            if cls_score is not None:
                ...
                if cls_score.numel() > 0:
                    ...
                    ### modified by cpf
                    # if 'neg_ins_score' in others:
                    #     pos_neg_loss, _, _ = self.loss_mil1(
                    #         cls_score,
                    #         ins_score,
                    #         labels + 1,
                    #         label_valid,
                    #         label_weights.unsqueeze(-1), )
                    #     pos_loss += pos_neg_loss
