...
class Shared2FCInstanceMILHeadEPLUS(Shared2FCInstanceMILHeadEP2B):
    ...
    def __init__(self,
        ...
        self.with_ori_distill=with_ori_distill
    ...
    def forward_head(self, x, stage, others=None):
        ...
        reg_box = self.fc_reg[0](x_reg) if self.with_distill and stage == self.with_distill - 1 else None
        # reg_box= None
        if self.with_others:
            others = dict(x_feat=x_cls)
            return cls_score, ins_score, reg_box, others
        else:
            return cls_score, ins_score, reg_box, dict()
