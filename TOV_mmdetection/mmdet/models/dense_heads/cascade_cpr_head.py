...
class CascadeCPRHead(AnchorFreeHead):
    ...
    def forward(self, feats):
        ...
        for cpr in self.cpr_list:
            res.append(cpr(feats))
        return res,

    def update_with_refine_outs(self, refine_outs, gt_r_bboxes, gt_labels, gt_weights, not_refine):
        ...
        return gt_r_bboxes, gt_weights, not_refine
    ...
    def loss(self, feats, gt_bboxes, gt_labels, img_metas, gt_bboxes_ignore=None, gt_true_bboxes=None):
        ...
        for i, cpr in enumerate(self.cpr_list):
            ...
            refine_res, cur_not_refine = cpr.get_bboxes(
                *feats_detach[i], img_metas, rescale=False, gt_bboxes=gt_r_bboxes,
                ...
                gt_anns_id=gt_anns_id, not_refine=not_refine, cascade_out_fmt=True,
            )

            gt_r_bboxes, gt_weights, not_refine = self.update_with_refine_outs(
                (refine_res, cur_not_refine), gt_r_bboxes, gt_labels, gt_weights, not_refine)
    ...
    def get_bboxes(self, feats, img_metas,
        ...
        for i, cpr in enumerate(self.cpr_list):
            ...
            gt_r_bboxes, _, not_refine = self.update_with_refine_outs(
                (refine_res, cur_not_refine), gt_r_bboxes, gt_labels, None, not_refine)
        return refine_res
