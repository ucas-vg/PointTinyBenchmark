...
class WeightedHausdorffDistance():
    ...
    def forward(self, prob_map, gt_bboxes, gt_labels, img_metas):
        """
        ...
        # orig_sizes, resized_height, resized_width,
        orig_sizes = [list(i['ori_shape'][:2]) for i in img_metas]
        orig_sizes = torch.tensor(orig_sizes)
        resized_height, resized_width, = prob_map.size()[-2:]
        height, width = resized_height, resized_width
        gt = self.pseudo_bbox_to_center(gt_bboxes)
        ...
        all_img_locations = torch.from_numpy(cartesian([np.arange(resized_height),
                                                        np.arange(resized_width)]))
        # Convert to appropiate type
        all_img_locations = all_img_locations.to(device=prob_map.device,
                                                 dtype=torch.get_default_dtype())

        _assert_no_grad(gt)

        assert prob_map.dim() == 3 or 4, 'The probability map must be (B x H x W)'
        assert prob_map.size()[-2:] == (height, width), \
        ...
        for b in range(batch_size):
            for cls in range(prob_map.shape[1]):
                # One by one
                prob_map_b = prob_map[b, cls, :, :]
                gt_b = gt[b]
                if cls in gt_labels[b]:
                    gt_b = gt_b[gt_labels[b] == cls]
                else:
                    continue

                orig_size_b = orig_sizes[b, :]
                norm_factor = 1 / img_metas[b]['scale_factor'][:2]
                norm_factor = torch.tensor(norm_factor).unsqueeze(0).to(prob_map.device)
