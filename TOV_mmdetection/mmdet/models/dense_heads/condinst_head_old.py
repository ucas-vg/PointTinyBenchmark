...
class CondInstMaskHead(BaseModule):
    ...
    def _init_layers(self):
        ...
        self.class_embedding = nn.Embedding(
            self.num_classes, self.bbox_head_channels)
    ...
    def training_sample(self, pseudo_boxes, gt_labels, num_levels, others, cascade_weight=None, test=None):
        ...
        # x_feat = torch.cat([x_feat, class_feat], dim=1)
        # ########################################3

        param_preds = self.param_linear(x_feat)
        param_preds = torch.split(param_preds, batch_gt)
        ...
        return param_preds, coors, level_inds, img_inds, pseudo_boxes, cascade_weight
    ...
    def loss(self,
        ...
        gt_bitmasks_shake_list = self.get_targets(gt_bboxes, gt_masks, imgs,
                                                  ...
                                                  dynamic_weight,
                                                  pseudo_boxes_shake)
        if self.mil_proj or self.enlarge_rate:

            mask_logits_ref = mask_logits[:, 1, None]

            if self._iter % 2 == 0:
                mask_logits_ = mask_logits[:, 2, None]
                mask_scores = mask_logits[:, 2, None].sigmoid()
            else:
                mask_logits_ = mask_logits[:, 0, None]
                mask_scores = mask_logits[:, 0, None].sigmoid()
            # mask_logits_1 = mask_logits[:, 2, None]
            # mask_scores_1 = mask_logits[:, 2, None].sigmoid()
            if self.whole_map:
                mask_wholemap_scores = mask_logits[:, 2].sigmoid()
        ...
        if self.boxinst_enabled:
            ...
            #             1 + gt_bitmasks - bitmasks_weight).float()
            ...
            if pseudo_boxes_shake:
                loss_prj = []
                loss_pair = []
                loss_prj.append(loss_prj_term)
                loss_pair.append(loss_pairwise)
                for i in range(len(pseudo_boxes_shake)):
                    gt_bitmasks_shake = gt_bitmasks_shake_list[i]
                    gt_bitmasks_shake = torch.cat(gt_bitmasks_shake, dim=0)
                    gt_bitmasks_shake = gt_bitmasks_shake[gt_inds].unsqueeze(1).to(mask_scores.dtype)
                    loss_prj_term_shake = compute_project_term(mask_scores, gt_bitmasks_shake, dynamic_weight,
                                                               bitmasks_weight)
                    loss_prj_term_shake = loss_prj_term_shake * warmup_factor1
                    weights_shake = (img_color_similarity >= self.pairwise_color_thresh).float() \
                                    * gt_bitmasks_shake.float()
                    loss_pairwise_shake = (pairwise_losses * weights_shake).sum() / weights_shake.sum().clamp(min=1.0)
                    loss_pairwise_shake = loss_pairwise_shake * warmup_factor2
                    loss_prj.append(loss_prj_term_shake)
                    loss_pair.append(loss_pairwise_shake)
                loss_prj = torch.stack(loss_prj).mean()
                loss_pair = torch.stack(loss_pair).mean()
                losses.update({
                    "loss_prj_shake": loss_prj,
                    "loss_pairwise_shake": loss_pair,
                })
            else:
                losses.update({
    ...
    def get_bitmasks_from_boxes(self, gt_bboxes, padded_images, padded_image_masks, dynamic_weight,
        ...
        for i, per_img_gt_bboxes in enumerate(gt_bboxes):
            ...
            for j, per_box in enumerate(per_img_gt_bboxes):  # [x1,y1, x2, y2]
                ...
                bitmask_full_weight[int(per_box[1] - rate * bh): int(per_box[3] + rate * bh) + 1,
                int(per_box[0] - rate * bw): int(per_box[2] + rate * bw) + 1] += 1.0
