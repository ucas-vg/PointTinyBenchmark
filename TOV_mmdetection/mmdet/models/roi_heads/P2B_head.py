...
class P2BHead(StandardRoIHead):
    ...
    def _bbox_forward_train(self, x, proposal_list_base, proposals_list, proposals_valid_list, neg_proposal_list,
        ...
        #     retrain_weights = self._bac_assigner(torch.cat(proposals_list),pseudo_boxes)
        # else:
    ...
    def _bbox_forward(self, x, rois, gt_points, stage):
        ...
        cls_score, ins_score, reg_box, others = self.bbox_head(bbox_feats, stage)
    ...
    def _mask_forward_train(self, x, proposals_list, gt_bboxes, gt_labels, dynamic_weight,
        ...
        # mask_atten_map = mask_atten_map.permute(1,0,2,3)

        cls_segms_list, im_mask_list = self.mask_head.get_seg_masks(mask_results['mask_cls_pred'].softmax(dim=-1),
                                                                    proposals_list, gt_labels,
                                                                    img_metas, rescale=False, threshold=0.01)
        for i in range(len(im_mask_list)):
            ...
            cv2.imshow('ims2', img)
            cv2.imshow('ims1', np.array(im_mask_list[i][0].float().cpu()))
            cv2.waitKey()
    ...
    def _mask_forward(self, x, rois=None, pos_inds=None, bbox_feats=None):
        ...
        # if atten_map:
        # mask_atten_map = atten_map[:, :, 0, 1:].reshape(*atten_map.shape[:2])

        mask_results = dict(mask_cls_pred=mask_cls_pred, mask_ins_pred=mask_ins_pred, mask_feats=mask_feats,
                            mask_atten_map=atten_map[:, :, 0, 1:].reshape(*atten_map.shape[:2], *mask_feats.shape[
                                                                                                 -2:]) if atten_map else None)
        return mask_results
    ...
    def merge_box_single(self, cls_score, ins_score, dynamic_weight, gt_point, gt_label, proposals, feat, img_metas,
                         stage):
        if stage < self.num_stages - 1:
    ...
    def merge_box(self, bbox_results, proposals_list, proposals_valid_list, gt_labels, gt_true_bboxes, img_metas,
        ...
        boxes, filtered_boxes, filtered_scores, feat = multi_apply(self.merge_box_single, cls_scores, ins_scores,
                                                                   dynamic_weight_list,
                                                                   gt_true_bboxes,
                                                                   gt_labels,
                                                                   proposals_list, feat,
    ...
    #     iou = bbox_overlaps(bbox, gt_bbox)
    #     box_weight = (iou > 0.7).long()
    #     _, idx = iou.max(dim=1)
    #     box_weight[:,idx] = 1
    #     return box_weight.squeeze(-1)
    ...
    def simple_test(self,
        ...
        det_bboxes, det_labels, pseudo_bboxes,_ = self.simple_test_bboxes(
    ...
    def simple_test_bboxes(self,
        ...
        if self.with_atten:
            x = self.atten_pts_and_ftmap(x, gt_points, img_metas)
