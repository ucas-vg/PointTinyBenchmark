...
class MILMaskHead(BaseModule):
    ...
    def __init__(self,
        ...
        super(MILMaskHead, self).__init__(init_cfg)
        ...
        self.pos_embed = nn.Parameter(torch.randn(1, 14 * 14, 256) * .02)

    def init_weights(self):
        super(MILMaskHead, self).init_weights()
        for m in [self.upsample, self.conv_cls, self.conv_ins]:
            if m is None:
                continue
    ...
    def forward(self, x):
        ...
        return mask_cls_pred, mask_ins_pred, None
    ...
    def loss(self, mask_cls_pred, mask_ins_pred, labels, label_weights):
        mask_cls_pred = mask_cls_pred.reshape(*mask_cls_pred.shape[:2], -1).permute(0, 2, 1)
        mask_ins_pred = mask_ins_pred.reshape(*mask_ins_pred.shape[:2], -1).permute(0, 2, 1)
        losses = dict()
        avg_factor = max(torch.sum(label_weights > 0).float().item(), 1.)
        if mask_cls_pred.size(0) > 0:
            if self.class_agnostic:
                loss_mask = self.loss_mask(mask_pred, mask_targets,
                                           torch.zeros_like(labels))
            else:
                label_valid = mask_cls_pred.new_full(
                    (*mask_cls_pred.shape[:2], 1), 1, dtype=torch.long)
                mask_cls_pred = mask_cls_pred.softmax(dim=-1)
                # cls_score=cls_score.sigmoid()
                pos_loss, bag_acc, num_pos = self.loss_mil(
                    mask_cls_pred,
                    mask_ins_pred,
                    labels,
                    label_valid,
                ...
                # label_weights_ = (label_valid.sum(dim=1) > 0).float()*label_weights.unsqueeze(-1)
                # num_sample = max(torch.sum(label_weights_.sum(dim=-1) > 0).float().item(), 1.)
                if isinstance(pos_loss, dict):
                    losses.update(pos_loss)
                else:
                    losses['loss_mask_mil'] = pos_loss
                losses['mask_bag_acc'] = bag_acc
        return losses
    ...
    def get_seg_masks(self, mask_pred, det_bboxes, det_labels,
        ...
        mask_atten_map=mask_pred[range(len(mask_pred)),torch.cat(det_labels)].unsqueeze(1)
