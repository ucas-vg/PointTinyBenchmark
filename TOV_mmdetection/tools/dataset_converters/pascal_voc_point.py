...
def parse_xml(args):
    ...
    for obj in root.findall('object'):
        ...
        point = [float(obj.find('point').find('x').text),float(obj.find('point').find('y').text)]
        bbox = [
...
def cvt_annotations(devkit_path, years, split, out_file):
    ...
    for year in years:
        ...
        xml_paths = [
            osp.join(devkit_path, f'VOC{year}/Annotations-QC-0-0-0.25-0.25-0.25/{img_name}.xml')
...
def cvt_to_coco_json(annotations):
    ...
    def addAnnItem(annotation_id, image_id, category_id, bbox, point, difficult_flag):
        ...
        xywh_pseudo = np.array([point[0] - 16, point[1] - 16, 32, 32])
        ...
        annotation_item['true_bbox'] = xywh.astype(int).tolist()
        annotation_item['bbox'] = xywh_pseudo.astype(float).tolist()
        annotation_item['point'] = point.astype(float).tolist()
    ...
    for ann_dict in annotations:
        ...
        for bbox_id in range(len(bboxes)):
            bbox = bboxes[bbox_id]
            point = points[bbox_id]
            label = labels[bbox_id]
            annotation_id = addAnnItem(
                annotation_id, image_id, label, bbox, point, difficult_flag=0)

        bboxes_ignore = ann['bboxes_ignore'][:, :4]
        points_ignore = ann['points_ignore']
        labels_ignore = ann['labels_ignore']
        for bbox_id in range(len(bboxes_ignore)):
            bbox = bboxes_ignore[bbox_id]
            point = points_ignore[bbox_id]
            label = labels_ignore[bbox_id]
            annotation_id = addAnnItem(
                annotation_id, image_id, label, bbox, point, difficult_flag=1)
